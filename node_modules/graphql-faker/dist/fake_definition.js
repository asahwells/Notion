...
exports.ValidationErrors = exports.buildWithFakeDefinitions = void 0;
const graphql_1 = require("graphql");
// FIXME
const validate_1 = require("graphql/validation/validate");
const fakeDefinitionAST = graphql_1.parse(/* GraphQL */ `
  enum fake__Locale {
    az
    cz
    de
...
const schemaWithOnlyFakedDefinitions = graphql_1.buildASTSchema(fakeDefinitionAST);
// FIXME: mark it as valid to be able to run `validate`
schemaWithOnlyFakedDefinitions['__validationErrors'] = [];
function buildWithFakeDefinitions(schemaSDL, extensionSDL, options) {
    var _a, _b, _c;
    const skipValidation = (_a = options === null || options === void 0 ? void 0 : options.skipValidation) !== null && _a !== void 0 ? _a : false;
    ...
    schema = new graphql_1.GraphQLSchema(Object.assign(Object.assign({}, config), (config.astNode ? {} : getDefaultRootTypes(schema))));
        ...
        for (const type of Object.values(schema.getTypeMap())) {
            if (graphql_1.isObjectType(type) || graphql_1.isInterfaceType(type)) {
                for (const field of Object.values(type.getFields())) {
                    const isExtensionField = ((_c = (_b = field.astNode) === null || _b === void 0 ? void 0 : _b.loc) === null || _c === void 0 ? void 0 : _c.source) === extensionSDL;
                    if (field.extensions) {
                        field.extensions['isExtensionField'] = isExtensionField;
                    }
                    else {
    ...
    return schema;
    function extendSchemaWithAST(schema, extensionAST) {
        if (!skipValidation) {
            const errors = [
                ...validate_1.validateSDL(extensionAST, schema),
                ...graphql_1.validate(schemaWithOnlyFakedDefinitions, extensionAST, [
                    graphql_1.ValuesOfCorrectTypeRule,
                ]),
            ];
            if (errors.length !== 0) {
                throw new ValidationErrors(errors);
        ...
        return graphql_1.extendSchema(schema, extensionAST, {
            assumeValid: true,
            commentDescriptions: true,
        });
    }
}
exports.buildWithFakeDefinitions = buildWithFakeDefinitions;
